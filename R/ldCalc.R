...
ldCalc <- function(gds,
  ...
  gds_was_filename <- F
  if(is.character(gds) && file.exists(gds)) { gds_was_filename <- T; gds <- seqOpen(gds) }
  seqFilterPush(gds) # Save user's pre-existing filters to restore it on exit or error.

  # A big tryCatch block is ugly, but necessary so user will never be left with a dangling gds file pointer or half-filtered file.
  tryCatch({
      ...
      # TODO: filter by MAC/MAF/miss. Useful if someone else is calculating LD for you.
        # NOTE: must be done AFTER filtering by samples
  ...
  tryCatch({
      ...
      if(!is.null(ref_alleles)) {
        # Given IDs/alleles and those in the GDS file not in same order, so match them.
